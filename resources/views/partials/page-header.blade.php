...
@php
    $headersDb = json_decode(\App\Models\Setting::get('headers', '[]'), true);
    $resolvedImage = $headersDb[$image] ?? $image;
@endphp

<section class="relative flex min-h-[320px] items-center justify-center overflow-hidden sm:min-h-[380px]">
    <div class="absolute inset-0 bg-cover bg-center bg-no-repeat"
         style="background-image: url('{{ str_starts_with($resolvedImage, 'http') ? $resolvedImage : asset($resolvedImage) }}');"></div>
